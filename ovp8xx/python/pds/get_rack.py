...
def callback(frame: Frame) -> None:
    ...
    if frame.has_buffer(buffer_id.O3R_RESULT_JSON):
        ...
        if parsed_json_array["getRack"]["detectionValid"]:
            print(
                f"Detected Rack: {json.dumps(parsed_json_array['getRack'], indent=4)}"
            )
        else:
            flags = parsed_json_array["getRack"].get("flags", 0)
            descriptions = parsed_json_array["getRack"].get("flagDescription", [])
            active_flags = [
                desc
                for i, desc in enumerate(descriptions)
                if (flags & (1 << i)) and desc
            ]

            print("No Rack detected")
...
def main(ip: str, extrinsics_config: str, app_config: str) -> None:
    ...
    # Trigger the PDS Application to detect Rack(s)
    print("Triggering PDS application to detect Rack")
...
if __name__ == "__main__":
    ...
    APP_CONFIG_FILE = "configs/pds_getRack.json"
    main(
        ip=IP,
