...
def tof_callback(frame: Frame) -> None:
    """Callback function executed every time a frame is received.
    The depth/amplitude data can be decoded and further processed.

    Args:
        frame (Frame): The frame containing the data for requested buffer ids
    """
    if frame.has_buffer(buffer_id.NORM_AMPLITUDE_IMAGE):
        amplitude = frame.get_buffer(buffer_id.NORM_AMPLITUDE_IMAGE)
        print(f"Norm Amplitude image is received of size {amplitude.shape}")
...
def callback(frame: Frame) -> None:
    ...
    if frame.has_buffer(buffer_id.O3R_RESULT_JSON):
        ...
        print(
            f"Detected pallet(s): {json.dumps(parsed_json_array['getPallet'], indent=4)}"
        )
...
def main(ip: str, extrinsics_config: str, app_config: str) -> None:
    ...
    camera_port = next(
        iter(pds_configuration["applications"]["instances"][f"{app_instance}"]["ports"])
    )
    ...
    # Create the framegrabber instance for the port
    port_fg = FrameGrabber(o3r, o3r.port(f"{camera_port}").pcic_port)
    port_fg.start([buffer_id.NORM_AMPLITUDE_IMAGE])
    port_fg.on_new_frame(tof_callback)
    ...
    o3r.set(
        {
            "applications": {
                "instances": {
                    f"{app_instance}": {
                        "configuration": {"customization": {"command": "getPallet"}}
                    }
                }
            }
        }
