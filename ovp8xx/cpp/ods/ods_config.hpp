...
private:
  ...
  static void CheckJSONFormat(const std::string &format,
                              const std::string &value) {
    // This is necessary because "format" is a keyword both used internally by
    // the schema validator and in the O3R schema.
    if (format == "ipv4") {
      std::clog << "IPV4 formatting";
      // if
      // (!std::get<0>(ifm::eucco::network::normalizeIPv4Address(QString::fromStdString(value))))
      // {
...
public:
  ...
  Validator(ifm3d::O3R::Ptr o3r_) : o3r(o3r_) {
    std::clog << "JSON validation unavailable: missing dependency."
              << std::endl;
  }
  void ValidateJson(ifm3d::json config) {
...
public:
  ...
  void SetConfigFromFile(const std::string &config_path) {
    ...
    std::clog << "Reading configuration file at: " << config_path << std::endl;
  ...
  void SetConfigFromStr(const std::string &config_str) {
    try {
      ...
      std::cerr << "Caught exception while configuring: " << e.what()
                << std::endl;
    }
  }
};
