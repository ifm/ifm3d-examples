...
int getFrameTime(ifm3d::FrameGrabber::Ptr fg, ifm3d::TimePointT &t,
                 int &ready) {
  /**
...
int main() {
  ...
  // Set framerate:
  //////////////////////////
  o3r->Set(ifm3d::json::parse(
      R"({"ports":{"port2":{"acquisition": {"framerate": 10}}, 
  ...
  if (abs(std::chrono::duration_cast<std::chrono::milliseconds>(timestamps[0] -
                                                                timestamps[1])
              .count()) >= 50) {
  ...
  while (true) {
    ...
    if (ready0 == 1 && ready1 == 1) {
      ...
      fmt::print(
          "Delay (ms): {}\n",
          abs(std::chrono::duration_cast<std::chrono::milliseconds>(t0 - t1)
                  .count()));
    }
  }
