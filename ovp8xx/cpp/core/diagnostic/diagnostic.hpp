...
public:
  ...
  ifm3d::json diagnostic =
      ifm3d::json::parse(R"({"id": "None", "message": "None"})");

  O3RDiagnostic(ifm3d::O3R::Ptr o3r_, bool log_to_file_ = false,
                const std::string &file_name_ = "")
      : o3r(o3r_), log_to_file(log_to_file_),
...
private:
  void AsyncDiagCallback_(int id_, std::string message_) {
    ...
    std::clog << "\n//////////////////////////////////" << std::endl;
    std::clog << "Id: " << diagnostic["id"] << std::endl;
    std::clog << "Message: " << diagnostic["message"] << std::endl;
  }
};
