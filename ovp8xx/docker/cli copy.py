...
def SSH_handles(username: str = "oem", password: str = "oem", private_key_path: str = None, IP: str = DEFAULT_IP, port: int = 22, remove_known_host: bool = True) -> Tuple[SSHClient, SCPClient]:
    """
    ...
    try:
        ...
        if "timed out" in str(FailureToConnectError):
            logger.info(
                f"Attempt to connect to {username}@{IP}:{port} timed out.")
...
if __name__ == "__main__":
    ...
    user = "oem"
    host = "192.168.0.69"
    id = "~/.ssh/id_rsa_ovp8xx"
    # ret, res = cli_passthrough(f"ssh -i {id} {user}@{host} docker image ls") 

    # %%
    cmd = "L4T_VERSION=32.7.4 CUDA_VERSION=10.2 PYTHON_VERSION=3.8 ./jetson-containers/jetson-containers build --skip-tests all --name dind_base docker"

    cmd = f'docker exec -it dind /bin/bash -c "{cmd}"'
    cmd = "docker attach python_logging"
    cmd = "dir"
    cmd = "docker psadsfa"
    # cmd = f"ssh -i {id} {user}@{host} {cmd}"
    # ret, res = cli_passthrough(cmd)
    print(cmd)
    # %%
    ...
    ssh, scp = SSH_handles(
        IP=host, password="oem", private_key_path="C:/Users/usdagest/.ssh/id_rsa_ovp8xx", remove_known_host=False)
    ssh: SSHClient = ssh
