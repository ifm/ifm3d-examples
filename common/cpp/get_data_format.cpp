...
int main() {
  ...
  if (device_type == ifm3d::Device::device_family::O3R) {
    ...
    for (const auto &port : o3r->Ports()) {
      if (port.type == "3D") {
        std::cout << "Using first available 3D port: " << port.port
                  << std::endl;
  ...
  } else if (device_type == ifm3d::Device::device_family::O3D or
             device_type == ifm3d::Device::device_family::O3X) {
  ...
  std::cout << "Data format: " << pixel_format_to_string(xyz.dataFormat())
            << std::endl;
