...
bool writePPMFile(ifm3d::Buffer &img, std::string const& filename)
{
	...
	for (auto col = size_t{ 0 }; col < write_height; ++col) {
		for (auto row = size_t{ 0 }; row < write_width; ++row) {
			write_pixels[pixel_index * 3 + 0] = static_cast<uint8_t>(*(img.ptr(0) + pixel_index));
			write_pixels[pixel_index * 3 + 1] = static_cast<uint8_t>(*(img.ptr(0) + pixel_index));
			write_pixels[pixel_index * 3 + 2] = static_cast<uint8_t>(*(img.ptr(0) + pixel_index));
			++pixel_index;
		}
	}
	try
...
template <typename  T>
void scaleImageToRGB(ifm3d::Buffer &input, ifm3d::Buffer &confidence, ifm3d::Buffer &output, double min = 0.0f, double max = 0.0f)
{
	output = input;
...
template < typename  T >
void findMinAndMax(ifm3d::Buffer &input, ifm3d::Buffer &confidence, double &min, double &max)
{
	max = 0;
	min = (double)INT_MAX;
	for (int index = 0; index < input.width() * input.height(); index++)
...
int main(int argc, const char **argv)
{
	...
	fg->Start({ifm3d::buffer_id::CONFIDENCE_IMAGE,ifm3d::buffer_id::NORM_AMPLITUDE_IMAGE,ifm3d::buffer_id::RADIAL_DISTANCE_IMAGE,ifm3d::buffer_id::CARTESIAN_ALL});
	...
	auto distance = frame.get()->GetBuffer(ifm3d::buffer_id::RADIAL_DISTANCE_IMAGE);
