...
//
// Prerequisites:
// 1) Each ifm 3D camera should be configured to use "Process Interface" for trigger.
// 2) You Should be able to ping each of the 3D camera from the PC on which this code executes.
// 3) Incase your network uses a proxy, you will need to configure your system to bypass the proxy for the used IP's.
//
...
namespace
{
    ...
    std::vector<std::string> camera_ip_list = {
        "192.168.0.68",
        "192.168.0.80"
    };
...
int main(int argc, const char **argv)
{
    ...
    for(int i=0;i<frame_count;i++)
    {
        ...
        for(const auto& camera_ip : camera_ip_list)
        {
            ...
            {
                std::cerr << "timeout waiting for camera(" << camera_ip << ") frame" << std::endl;
                return -1;
            }
            else
            {
        ...
        std::cout << "total time taken to receive in ms " << duration_ms.count() << " ms" << std::endl;
    }
return 0;
}