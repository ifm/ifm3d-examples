...
template <std::size_t N, typename T> double timeit(T func) {
...
int main(int argc, const char **argv) {
  ...
  auto acquire_frame = [fg](cv::Mat &cloud, int resolution,
                            bool sw = false) -> void {
    for (int i = 0; i < 5; ++i) {
  ...
  std::cout << "Test 0: Streaming mode, toggling 23K <-> 100k " << N << "x ("
            << 2 * N << " switches)" << std::endl;
  double switch0 =
  ...
  // optional - uncomment below line of code in case device does not exit from
  // edit-mode session
  ...
  std::cout << "Test 1: S/W trigger mode, toggling 23K <-> 100k " << N << "x ("
            << 2 * N << " switches)" << std::endl;
  double switch1 =
      timeit<N>([&res, pcic, assert_pcic_ok, acquire_frame, &cloud]() {
        res = pcic->Call("a02");
