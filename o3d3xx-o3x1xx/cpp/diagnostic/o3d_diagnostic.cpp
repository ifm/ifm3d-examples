...
std::tuple<int32_t, int32_t, int32_t, int32_t, uint32_t, uint32_t>
unpackData(ifm3d::Buffer_<std::uint8_t> &data) {
  int32_t i1, i2, i3, i4;
  uint32_t u1, u2;

  // Assuming data is at least 24 bytes: 4 ints (4*4 bytes) + 2 uints (2*4
  // bytes)
...
int main() {
  ...
  auto fg = std::make_shared<ifm3d::FrameGrabber>(
      o3d, std::stoi(o3d->DeviceParameter("PcicTcpPort")));
  ...
  std::cout << "Illumination temperature (0.1 °C), invalid = 32767: "
            << std::get<0>(unpacked_diagnostic) << std::endl;
  std::cout << "Frontend temperature 1 (0.1 °C), invalid = 32767: "
            << std::get<1>(unpacked_diagnostic) << std::endl;
  std::cout << "Frontend temperature 2 (0.1 °C), invalid = 32767: "
            << std::get<2>(unpacked_diagnostic) << std::endl;
  std::cout << "i.mx6 Temperature (0.1 °C), invalid = 32767: "
            << std::get<3>(unpacked_diagnostic) << std::endl;
  std::cout << "Frame duration: " << std::get<4>(unpacked_diagnostic)
            << std::endl;
  std::cout << "Framerate: " << std::get<5>(unpacked_diagnostic) << std::endl;
  return 0;
}
