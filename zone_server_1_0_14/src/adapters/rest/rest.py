...
from ..data_models import AdHocZoneSetting, ZoneSet
...
class Frontend:
    ...
    def __init__(self, to_frontend, to_backend, log_level, port):
        ...
        openapi_schema = get_openapi(
            title="Zone Server API",
            version="2.0.0",
            description="Control your VPU's ODS instance",
    ...
    async def sync(self):
        ...
        self.n_requests += 1
        logging.info(f"Updating state for REST api from: {self.state}")
        self.update_state()
        logging.info(f"Updated state for REST api to: {self.state}")
        return self.state
...
class Adapter:
    ...
    def __init__(self, log_level="WARNING", port=DEFAULT_PORT, **kwargs):
        ...
        self.backend_state = {"odsMode": "IDLE", "occupancy": [1, 1, 1]}
        self.push()
